...
def buildurl(marca,GType,makm,mikm,mapr,miy,txt):
    ...
    url2 = "http://www.coches.net/"+marca+"/segunda-mano/?pg=1&FuelTypeId="+str(GasType)+"&MaxKms="+str(makm)+"&MinKms="+str(mikm)+"&MinYear="+str(miy)+"&text="+str(txt)
    page = urlopen(url2)
    soup = BeautifulSoup(page,"lxml")
    iterations = soup.find('div',attrs={"class":"mt-SerpHeader u-flex--spaceBetween"}).find('h1').string
